...
namespace TexasHoldemBonusSimulator
{
    internal class Program
    {
        ...
        private static long EvaluatePreflopTree(int ante)
        {
            ...
            foreach (DistinctHolding holding in holdingMap.Values.Skip(10))
            {
        ...
        private static long EvaluateFlopTree(List<Card> playerCards, int ante)
        {
            ...
            int holdingsAmount = holdingMap.Values.Count;
            int holdingsDone = 0;
            long winnings = 0;

            Console.WriteLine(holdingsAmount);
            ...
            foreach (DistinctHolding holding in holdingMap.Values)
            {
                ...
                if (holdingsDone % 10 == 0)
                {
                    Console.WriteLine($"Progress: {holdingsDone / (double)holdingsAmount * 100: 0.00}%");
